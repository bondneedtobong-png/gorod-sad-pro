...
function Pill({ children }: { children: React.ReactNode }) {
  return (
    <span className="inline-flex items-center rounded-full bg-white/5 px-2.5 py-1 text-[11px] font-medium text-mist/70 ring-1 ring-white/10">
      {children}
    </span>
  );
}
  ...
  return (
    <div>
      ...
      <div className="relative mb-5 max-w-md">
        <Search className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-mist/45" />
        <input
          type="search"
          value={query}
          ...
          className="w-full rounded-full border border-white/12 bg-white/5 py-3 pl-11 pr-4 text-sm text-mist placeholder:text-mist/40 shadow-sm transition focus:border-aqua-400 focus:outline-none focus:ring-2 focus:ring-aqua-400/30"
        />
      </div>
      ...
      <div className="mb-8 space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          <span className="mr-1 inline-flex items-center gap-1.5 text-xs uppercase tracking-[0.18em] text-mist/50">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <span className="mr-1 text-xs uppercase tracking-[0.18em] text-mist/50">
          ...
          {hasFilters && (
            <button
              type="button"
              onClick={reset}
              className="inline-flex items-center gap-1 rounded-full px-3 py-1.5 text-xs font-medium text-mist/60 underline-offset-2 transition hover:text-aqua-400 hover:underline"
            >
      ...
      {/* Счётчик */}
      <div className="mb-5 text-sm text-mist/60">
        Найдено: <span className="font-semibold text-mist">{filtered.length}</span>{" "}
        из {plants.length}
      </div>
      ...
      {filtered.length > 0 ? (
            ...
            <Reveal key={p.slug} delay={Math.min(i, 5) * 60}>
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <p className="mt-2 text-sm leading-relaxed text-mist/70">{p.short}</p>
        ...
        <div className="rounded-3xl border border-dashed border-white/15 bg-white/[0.03] px-6 py-16 text-center">
          <div className="font-display text-2xl text-mist">Ничего не найдено</div>
          <p className="mx-auto mt-2 max-w-sm text-sm text-mist/60">
          ...
          {hasFilters && (
            <button
              type="button"
              onClick={reset}
              className="mt-5 inline-flex items-center gap-1.5 rounded-full bg-gs-fresh px-5 py-2.5 text-sm font-semibold text-pine-950 shadow-aqua-glow transition hover:brightness-110"
            >
  ...
  return (
    <button
      ...
      className={cn(
        ...
        active
          ? "bg-aqua-400 text-pine-950 ring-aqua-400 shadow-aqua-glow"
          : "bg-white/5 text-mist/75 ring-white/10 hover:ring-aqua-400/50 hover:text-aqua-400",
      )}
    >
      {children}
    </button>
